...
if (!$order_by) {
 ...
 $db = getDbInstance();
    ...
    $db->join("conference n", "c.conf_id=n.id", "INNER");
    $db->join("union_mission m", "c.union_id=m.id", "INNER");
   
//Get DB instance. i.e instance of MYSQLiDB Library

if ($_SESSION['user_type'] == 'auditor') {
  $db->where("c.conf_id", $_SESSION['conference']);
  
   
}

if ($_SESSION['user_type'] == 'union_auditor') {
   $db->where("c.union_id",  $_SESSION['union_mission']);
}
 $select = array('c.id', 'n.conf_name', 'u.user_name', 'c.name', 'c.mobile', 'm.union_name', 'c.date');
//
...
<?php foreach ($church as $row) : ?>
                <tr>
                    ...
                    <td data-name="conf_id" data-original-title="Select Conference" id="conference" class="conference_id" data-type="select" data-pk="<?php echo $row['id'] ?>"><?php echo htmlspecialchars($row['conf_name']) ?></td>
