...
// Page header
        ...
        $q1 = "SELECT * FROM estimated_income WHERE church_id=$church_id AND date  BETWEEN '$from' AND '$to' ";
   	...
   	header('location: ../../estimatedIncomePdf.php');
         exit;
        } 
if(!$error){
        $count = "SELECT source_name, amount, date from estimated_income WHERE church_id='$church_id' AND date  BETWEEN '$from' AND '$to' ORDER BY date DESC";
        $totalIncome = " SELECT SUM(amount) AS totalIncome from estimated_income WHERE church_id='$church_id' AND date  BETWEEN '$from' AND '$to' ";
...
/// each record is one row  ///
       ...
       $pdf->Output('estimatedIncomePdf.pdf','D');
